  ...
  async verifySession(
    ...
    try {
      ...
      return {
        ...
        appId: typeof appId === "string" && appId ? appId : "radhe-radhe-restaurant",
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    if (!user && ENV.oAuthServerUrl) {
      try {
        ...
        console.warn("[Auth] OAuth sync skipped:", error);
      }
    }

    if (!user) {
      user = await db.findUserByIdentifier(sessionUserId);
    }
